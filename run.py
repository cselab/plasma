...
def stitch_state_files(file_restart: file_restart_pydantic_model.FileRestart,
  ...
  """
    ...
    previous_datatree = previous_datatree.sel(
        time=slice(None, datatree.time[0]))
...
class StateHistory:
    ...
    def _save_core_transport(self, ) -> dict[str, xr.DataArray | None]:
        ...
        core_transport = self._stacked_core_transport
        if (core_transport.chi_face_el_bohm is not None
...
def get_initial_state_and_post_processed_outputs_from_file(
        t_initial, file_restart, runtime_params_provider, geometry_provider,
        step_fn):
    ...
    post_processed_outputs = dataclasses.replace(
        ...
        E_external_injected=scalars_dataset.data_vars['E_external_injected'].
        to_numpy(),
        E_external_total=scalars_dataset.data_vars['E_external_total'].
        to_numpy(),
    )
...
def _override_initial_runtime_params_from_file(runtime_params, geo, t_restart,
                                               profiles_ds):
    # pylint: disable=invalid-name
    runtime_params.numerics.t_initial = t_restart
    runtime_params.profile_conditions.Ip = profiles_ds.data_vars[
        output.IP_PROFILE].to_numpy()[-1]
    runtime_params.profile_conditions.T_e = (
        profiles_ds.data_vars[output.T_E].sel(
            rho_norm=profiles_ds.coords[output.RHO_CELL_NORM]).to_numpy())
    runtime_params.profile_conditions.T_e_right_bc = (
        profiles_ds.data_vars[output.T_E].sel(
            rho_norm=profiles_ds.coords[output.RHO_FACE_NORM][-1]).to_numpy())
    runtime_params.profile_conditions.T_i = (
        profiles_ds.data_vars[output.T_I].sel(
            rho_norm=profiles_ds.coords[output.RHO_CELL_NORM]).to_numpy())
