...
def compute_right_face_value(
    value, dr, right_face_constraint, right_face_grad_constraint
):
...
def compute_face_value(value, dr, right_face_constraint, right_face_grad_constraint):
    ...
    right_face = compute_right_face_value(
        value, dr, right_face_constraint, right_face_grad_constraint
    )
    return jnp.concatenate([left_face, inner, right_face], axis=-1)


def compute_cell_plus_boundaries(
    value, dr, right_face_constraint, right_face_grad_constraint
):
    left_value = compute_left_face_value(value)
    right_value = compute_right_face_value(
        value, dr, right_face_constraint, right_face_grad_constraint
    )
...
def compute_face_value_bc(value, dr, bc):
    return compute_face_value(
        value, dr, bc["right_face_constraint"], bc["right_face_grad_constraint"]
    )
...
def make_convection_terms(v_face, d_face, dr, bc):
    ...
    vec_value = -v_face[0] * (1.0 - left_alpha[0]) * bc["left_face_grad_constraint"]
    ...
    if bc["right_face_constraint"] is not None:
        ...
        vec_value = (
            -2.0 * v_face[-1] * (1.0 - right_alpha[-1]) * bc["right_face_constraint"]
        ) / dr
    else:
        mat_value = -(v_face[-1] - v_face[-2] * left_alpha[-1]) / dr
        vec_value = (
            -v_face[-1] * (1.0 - right_alpha[-1]) * bc["right_face_grad_constraint"]
        )
...
def make_diffusion_terms(d_face, dr, bc):
    ...
    if bc["right_face_constraint"] is not None:
        ...
        vec = vec.at[-1].set(2 * d_face[-1] * bc["right_face_constraint"] / denom)
    else:
        diag = diag.at[-1].set(-d_face[-2])
        vec = vec.at[-1].set(d_face[-1] * bc["right_face_grad_constraint"] / dr)
...
def calculate_psidot_from_psi_sources(*, psi_sources, sigma, psi, psi_bc):
    ...
    conv_mat, conv_vec = make_convection_terms(
        v_face_psi, d_face_psi, jnp.array(g.dx), psi_bc
    )
...
def calculate_average_charge_state_single_species(T_e, ion_symbol):
    ...
    interval_indices = jnp.searchsorted(g.TEMPERATURE_INTERVALS[ion_symbol], T_e)
    Zavg_coeffs_in_range = jnp.take(
        g.MAVRIN_Z_COEFFS[ion_symbol], interval_indices, axis=0
    ).transpose()
    X = jnp.log10(T_e)
...
def calculate_f_trap():
    epsilon_effective = (
        0.67
        * (1.0 - 1.4 * jnp.abs(g.geo_delta_face) * g.geo_delta_face)
        * g.geo_epsilon_face
    )
...
def calculate_conductivity(n_e, T_e, Z_eff_face, q_face):
    ...
    sigsptz = 1.9012e04 * (T_e_face * 1e3) ** 1.5 / Z_eff_face / NZ / log_lambda_ei
...
def _calculate_bootstrap_current(
    *,
    ...
    T_i_ev = T_i_face * 1e3
    log_lambda_ii = (
        30.0 - 0.5 * jnp.log(n_i_face) + 1.5 * jnp.log(T_i_ev) - 3.0 * jnp.log(Z_i_face)
    )
...
def default_formula(
    unused_core_profiles, unused_calculated_source_profiles, unused_conductivity
):
    absorbed_power = g.generic_heat_P_total * 1.0
    profile = gaussian_profile(
        center=g.generic_heat_location, width=g.generic_heat_width, total=absorbed_power
    )
...
def fusion_heat_model_func(
    ...
    for fraction, symbol in zip(g.main_ion_fractions, g.main_ion_names):
        if symbol == "D" or symbol == "T":
            ...
            n_i_face_fusion = compute_face_value_bc(
                core_profiles.n_i, jnp.array(g.dx), core_profiles.n_i_bc
            )
            logPfus = (
                jnp.log(DT_fraction_product * Efus)
                + 2 * jnp.log(n_i_face_fusion)
                + logsigmav
            )
...
def build_source_profiles1(
    ...
    weighted_Z_eff = (
        n_i * Z_i**2 / A_i + n_impurity * Z_impurity**2 / A_impurity
    ) / n_e
    log_Qei_coef = (
        jnp.log(g.Qei_multiplier * 1.5 * n_e)
...
def calculate_transport_coeffs(
    ...
    T_i_face_grad_rmid = compute_face_grad_bc(
        T_i, jnp.array(g.dx), g.T_i_bc, x=g.geo_rmid
    )
    T_e_face = compute_face_value_bc(T_e, jnp.array(g.dx), g.T_e_bc)
    T_e_face_grad_rmid = compute_face_grad_bc(
        T_e, jnp.array(g.dx), g.T_e_bc, x=g.geo_rmid
    )
    n_e_face = compute_face_value_bc(n_e, jnp.array(g.dx), g.n_e_bc)
    n_e_face_grad_rmid = compute_face_grad_bc(
        n_e, jnp.array(g.dx), g.n_e_bc, x=g.geo_rmid
    )
    ...
    n_impurity_face_grad = compute_face_grad_bc(
        n_impurity, jnp.array(g.dx), n_impurity_bc, x=rmid
    )
    ...
    iota_scaled0 = jnp.expand_dims(jnp.abs(psi_face_grad[1] / jnp.array(g.dx)), axis=0)
    ...
    tau_bounce = (
        q_face * g.R_major / (epsilon**1.5 * jnp.sqrt(T_e_face * g.keV_to_J / g.m_e))
    )
    ...
    alpha = factor_0 * (
        T_e_face * n_e_face * (lref_over_lte + lref_over_lne)
        + n_i_face * T_i_face * (lref_over_lti + lref_over_lni0)
        + n_impurity_face * T_i_face * (lref_over_lti + lref_over_lni1)
    )
    smag = smag - alpha / 2
    ...
    chi_face_ion = (
        ...
        / qualikiz_inputs.lref_over_lti
    ) * qualikiz_inputs.chiGB
    chi_face_el = (
        ((gradient_reference_length / gyrobohm_flux_reference_length) * qe)
        / qualikiz_inputs.lref_over_lte
    ) * qualikiz_inputs.chiGB
    Deff = -pfe_SI / (n_e_face_grad * g.geo_g1_over_vpr2_face * g.geo_rho_b + g.eps)
    Veff = pfe_SI / (n_e_face * g.geo_g0_over_vpr_face * g.geo_rho_b)
    Deff_mask = (
        ((pfe >= 0) & (qualikiz_inputs.lref_over_lne >= 0))
        | ((pfe < 0) & (qualikiz_inputs.lref_over_lne < 0))
    ) & (abs(qualikiz_inputs.lref_over_lne) >= g.An_min)
    ...
    active_mask = (
        (g.face_centers > g.transport_rho_min)
...
def get_updated_ions(n_e, n_e_bc, T_e, T_e_bc):
    ...
    n_impurity_right_face_constraint = jnp.where(
        ...
        (
            ...
            - n_i_bc["right_face_constraint"] * Z_i_face[-1]
        )
        / Z_impurity_face[-1],
    )
    ...
    n_impurity_face = compute_face_value_bc(n_impurity, jnp.array(g.dx), n_impurity_bc)
    Z_eff_face = (
        Z_i_face**2 * n_i_face + Z_impurity_face**2 * n_impurity_face
    ) / n_e_face
...
psi_from_Ip += psi[0]
psi_from_Ip[-1] = psi_from_Ip[-2] + (16 * g.mu_0 * np.pi**3 * Phi[-1]) * Ip_chease[
    -1
] / (g2g3_over_rhon[-1] * F_chease[-1]) * (
...
g.geo_epsilon_face = (g.geo_R_out_face - g.geo_R_in_face) / (
    g.geo_R_out_face + g.geo_R_in_face
)
...
while True:
    ...
    while True:
        ...
        for _ in range(0, g.n_corrector_steps + 1):
            ...
            mask = jnp.zeros_like(g.geo_rho, dtype=bool).at[rho_norm_ped_top_idx].set(True)
            sigma, sigma_face = calculate_conductivity(n_e, T_e, ions.Z_eff_face, q_face)
            ...
            toc_psi = (
                1.0 / g.resistivity_multiplier * g.cell_centers * sigma * g.mu0_pi16sq_Phib_sq_over_F_sq
            )
            ...
            turbulent_transport = calculate_transport_coeffs(
                ...
                q_face, ions.A_i, ions.Z_eff_face,
            )
            ...
            geo_factor = jnp.concatenate([jnp.ones(1), g.geo_g1_over_vpr_face[1:] / g.geo_g0_face[1:]])
            ...
            chi_face_per_ion = jnp.where(g.face_centers > g.rho_norm_ped_top, 0.0, chi_face_per_ion)
            chi_face_per_el = jnp.where(g.face_centers > g.rho_norm_ped_top, 0.0, chi_face_per_el)
            v_heat_face_ion = T_i_face_grad / T_i_face * chi_face_per_ion
            v_heat_face_el = T_e_face_grad / T_e_face * chi_face_per_el
            d_face_per_el = jnp.where(g.face_centers > g.rho_norm_ped_top, 0.0, d_face_per_el * g.geo_g1_over_vpr_face)
            v_face_per_el = jnp.where(g.face_centers > g.rho_norm_ped_top, 0.0, v_face_per_el * g.geo_g0_face)
            ...
            var_to_toc = {"T_i": toc_T_i, "T_e": toc_T_e, "psi": toc_psi, "n_e": toc_dens_el}
            var_to_tic = {"T_i": tic_T_i, "T_e": tic_T_e, "psi": tic_psi, "n_e": tic_dens_el}
            ...
            var_to_d_face = {"T_i": full_chi_face_ion, "T_e": full_chi_face_el, "psi": d_face_psi, "n_e": full_d_face_el}
            d_face = tuple(var_to_d_face[var] for var in g.evolving_names)
            var_to_v_face = {"T_i": v_heat_face_ion, "T_e": v_heat_face_el, "psi": v_face_psi, "n_e": full_v_face_el}
            ...
            source_mat_cell = tuple(
                tuple(d_mat.get((row_block, col_block)) for col_block in g.evolving_names)
            ...
            if v_face is not None:
                for i in range(num_channels):
                    ...
                    d_face_i = (
                        jnp.zeros_like(v_face[i]) if d_face_i is None else d_face_i
                    )
                    (conv_mat, conv_vec) = make_convection_terms(
                        v_face[i], d_face_i, x[i][1], x[i][2]
                    )
                    c_mat[i][i] += conv_mat
                    c[i] += conv_vec
            ...
            out = [(value, var[1], var[2]) for var, value in zip(x_input, x_new)]
            x_new = tuple(out)
        ...
        take_another_step = solver_did_not_converge & (
            at_exact_t_final | ~next_dt_too_small
        )
